...
!pip install git+https://github.com/pathwaycom/pathway.git@main --upgrade

import pathway as pw
import ast

# Step 1: Define schema matching your CSV columns
...
# Step 2: Read the CSV (remove header/delimiter)
input_table = pw.io.csv.read(
    "final_embedded_chunks.csv",
    schema=InputSchema,
    mode="static"
)

# Step 3: Parse stringified embedding
@pw.udf
def parse_embedding(emb_str: str) -> list[float]:
    return ast.literal_eval(emb_str)

# Step 4: Process into cleaned vector store
vector_store = input_table.select(
    row_id=input_table.id,
    chunk=input_table.chunk,
    embedding=parse_embedding(input_table.embedding),
    url=input_table.url,
    title=input_table.title,
)

# Step 5: Write to new CSV
pw.io.csv.write(vector_store, "vector_store_final.csv")
...
!cp myfile.txt /content/drive/MyDrive/my_colab_files/